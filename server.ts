...
// --- SERVER-SIDE MANDELBROT & RICIS III FRACTAL RENDERER (HYBRID CLIENT-SERVER ENGINE) ---
function getColorRGBSimpleServer(t: number, maxI: number, smooth: boolean, scheme: string, theta: number): [number, number, number] {
...
function rgbaToBmpDataUrl(width: number, height: number, rgba: Uint8Array): string {
...
app.post('/api/mandelbrot/render', (req, res) => {
  try {
    const {
      cx = -0.7,
      cy = 0.0,
...
app.post('/api/admin/request-code', async (req, res) => {
  ...
  // Construct mailto link so user's mail client (Apple Mail, Outlook, Thunderbird, etc.) opens directly
  ...
  // Attempt direct SMTP dispatch if SMTP configuration is present in environment
  ...
  // Strictly do NOT return testCode or magicLink in JSON body!
  res.json({ 
    success: true, 
    message: isAuthor 
